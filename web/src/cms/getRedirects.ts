...
export async function getRedirects(): Promise<Record<string, RedirectConfig>> {
  ...
  if (!cmsUrl || !cmsApiKey) {
    console.warn('CMS_URL or CMS_API_KEY not defined, returning empty redirects')
    return {}
  }
  ...
  const payloadSDK = new PayloadSDK<Config>({
    ...
    baseInit: {
      headers: {
        Authorization: `api-keys API-Key ${cmsApiKey}`,
      },
    },
  })
  ...
  try {
    ...
    console.warn('Failed to fetch redirects from CMS:', err)
    return {}
  }

  if (!redirectsCms || !Array.isArray(redirectsCms.docs)) {
    console.warn('Unexpected response from CMS redirects API:', redirectsCms)
    return {}
  }
